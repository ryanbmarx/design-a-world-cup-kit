...
/**
 *
 * @param {string} sstsString - section/subsection/topic/subtopic
 * @param {object} options
 * @param {[]string} options.site_codes, An array of the desired properties whose content should be searched
 * @param {integer} options.size, How many stories do we want back?
 */

async function searchBySSTS(sstsString = "", { site_codes = ["USAT"], size = 5 }) {
	if (sstsString == "") return;

	const ssts = sstsString.split("/");
	...
	const props = ["sections", "subsections", "topics", "subtopics"];
	searchParameters = props.reduce((a, c, i) => {
		return ssts[i] ? { ...a, [c]: [ssts[i]] } : a;
	}, searchParameters);

	searchParameters = JSON.stringify(searchParameters);

	return sstsQuery({ searchParameters })
		.then(data => data.searchAssets.assets)
		.catch(e => {
			console.error(e);
			return [];
		});
...
const sstsQuery = gql(`query WorldCupStories($searchParameters: JSONObjectString) {
	searchAssets(template:"asset_by_ssts", parameters: $searchParameters) {
		numResults
		assets {
			id
			title
