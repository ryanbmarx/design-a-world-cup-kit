...
const PROJECT_SLUG = "design-a-kit";
...
async function renderHTML(content = {}) {
	...
	const sprite = await fs.readFile(
		path.resolve(__dirname, "./data/sprite.svg"),
		"utf-8"
	);

	return minify(
		`<div style="display:none;">${sprite}</div><div id="${PROJECT_SLUG}" class="app">${html}</div>`,
		{
			minifyCSS: true,
			minifyJS: true,
...
async function preview() {
	const [template, content] = await Promise.all([
		...
		fs
			.readFile(path.resolve(__dirname, "./data/content.json"), "utf-8")
			.then(JSON.parse),
	]);

	const rendered = template.replace("{{HTML}}", await renderHTML(content));
